...
이번 포스트에서는 파이썬에서 정규표현식을 이용해 특정 텍스트에서 원하는 패턴을 찾는 방법에 대해 알아보겠습니다.  

### 1. 정규표현식을 사용하지 않는다면...
먼저, 특정 문자열에서 전화번호 패턴을 찾고 싶다고 가정해 보겠습니다. 실제 전화번호는 다양한 패턴이 존재하지만, 이해의 편의를 위해 검색 패턴은 2개의 숫자, 하이픈, 세 개의 숫자, 하이픈, 네 개의 숫자 순으로 정의하겠습니다. 즉, 다음과 같을 것입니다. 02-730-5800(청와대 민원실 전화번호...). 문자열이 이러한 패턴과 일치하는지 여부를 확인하는 함수를 만들어 보겠습니다. 다음 소스코드는 실제로 이렇게 작성하면 좋지 않다는 것을 보여주기 위해 작성된 것입니다.
```
...
위 함수를 이용해, 긴 문자열에서 특정 패턴(02-XXX-XXXX)의 전화번호만 찾는 코드를 작성하려면 어떻게 해야 할까요? 다음과 같이 작성해 보겠습니다.
```
...
### 2. 정규표현식으로...
이번에는 정규표현식을 이용해 위와 동일한 패턴(02-XXX-XXXX)의 전화번호를 찾는 소스를 작성해 보겠습니다.
```
import re
...
정규표현식을 이용해 re.compile() 함수로 Regex객체를 만듭니다. (이때, 정규표현식의 문자열 첫번째 따옴표 앞에 r을 놓아 원시 문자열을 전달하였습니다.)  
...
**숫자2개** **하이푼1개** **숫자3개** **하이푼1개** **숫자4개** 패턴과 일치하는 텍스트를 찾는 정규표현식 입니다.
...
```
파이썬에서는 위와같이 복합한 정규표현식을 이해하기 쉽게 작성하기 위해 여러줄로 나누어 작성할 수 있습니다.
```
import re 
...
```
#### 이메일 주소 정규표현식
아래는, 거의 모든 일반적인 이메일 주소를 일치시키는 정규표현식 입니다.
```
import re   
